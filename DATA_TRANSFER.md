...
То же касается `.env.deploy.local`: после переноса проекта на новый компьютер этот файл нужно скопировать вручную или заново создать из `.env.deploy.example`.
...
- локальные `.env` и `.env.deploy.local` перенесены отдельно и не потеряны
