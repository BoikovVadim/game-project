...
import {
  getVisibleStageTotalsForProgress,
  normalizeProgressSnapshot,
  withReliableProgressCorrectCounts,
} from './domain/progress-read-model';
import {
...
export class TournamentsService implements OnModuleInit {
  ...
  private async loadReusableTournamentPool(
    ...
    for (const tournament of tournaments) {
      ...
      for (const participantId of this.getTournamentParticipantIds(
        tournament,
      )) {
    ...
    return tournaments.map((tournament) => {
      ...
      return {
        ...
        hasCurrentUser:
          args.userId != null
            ? (participantIds?.has(args.userId) ?? false)
            : false,
  ...
  async previewReusableTournamentSelection(args: {
    ...
    return {
      ...
      candidates: entries.map((entry) =>
        this.toReusableTournamentPreviewItem(entry),
      ),
    };
  }
  ...
  async repairTournamentConsistency(): Promise<{
    ...
    recomputedCorrectCountRows: number;
    skippedUnreliableCorrectCountRows: number;
    ...
    const correctCountRepair = await this.recomputeProgressCorrectCounts();
    ...
    return {
      ...
      recomputedCorrectCountRows: correctCountRepair.updatedProgressRows,
      skippedUnreliableCorrectCountRows:
        correctCountRepair.skippedUnreliableProgressRows,
  ...
  async recomputeProgressCorrectCounts(): Promise<{
    updatedProgressRows: number;
    skippedUnreliableProgressRows: number;
  }> {
    const allProgress = await this.tournamentProgressRepository.find();
    const tournamentIds = [
    ...
    if (tournamentIds.length === 0) {
      return { updatedProgressRows: 0, skippedUnreliableProgressRows: 0 };
    }
    ...
    for (const question of questions) {
      const questionTournamentId = question.tournament?.id;
      if (!questionTournamentId) continue;
      const bucket = questionsByTournamentId.get(questionTournamentId) ?? [];
      bucket.push(question);
      questionsByTournamentId.set(questionTournamentId, bucket);
    }

    let updatedProgressRows = 0;
    let skippedUnreliableProgressRows = 0;

    for (const progress of allProgress) {
      const answersChosen = normalizeChosenAnswers(progress.answersChosen);
      if (answersChosen.length === 0) continue;
      if (!hasReliableChosenAnswers(answersChosen)) {
        skippedUnreliableProgressRows += 1;
        continue;
      }
      ...
      const effectivePlayerOrder =
        Array.isArray(tournament.playerOrder) &&
        tournament.playerOrder.length > 0
          ? tournament.playerOrder
      ...
      let changed = false;
      if (progress.correctAnswersCount !== recomputed.total) {
      ...
      if (
        ...
        progress.semiFinalCorrectCount !== recomputed.semi
      ) {
        progress.semiFinalCorrectCount = recomputed.semi;
        changed = true;
      }
  ...
  async convertLegacyMoneyTournamentsWithoutLeagueToTraining(): Promise<{
    ...
    const tournamentIds = legacyMoneyTournaments.map(
  ...
  private async syncTournamentActiveStatusWithManager(
    ...
    await manager
      ...
      .update({ id: tournamentId }, { status: TournamentStatus.ACTIVE });
    return true;
  }
  ...
  private async processTournamentEscrow(tournamentId: number): Promise<void> {
    ...
    const settlement =
      await this.getMoneyTournamentSettlementResolution(tournamentId);
  ...
  async startTraining(userId: number): Promise<{
    ...
    return this.tournamentRepository.manager.transaction(async (manager) => {
      ...
      const reusableTournaments = await this.loadReusableTournamentPool(
        manager,
        {
          gameType: 'training',
          userId,
      ...
      const waitingTournament =
        this.pickReusableTournamentEntry(reusableTournaments)?.tournament ??
        null;
  ...
  async getPlayersOnlineByLeague(): Promise<Record<number, number>> {
    ...
    for (const t of tournaments.filter(
      (tournament) => this.getTournamentPlayerCount(tournament) < 4,
    )) {
  ...
  async getMyTournaments(
    ...
    const questionsByTournamentId =
      await this.loadQuestionsByTournamentId(allIds);
    ...
    if (allIds.length > 0) {
      ...
      const getOppositeFinalistProgress = (
        ...
        const pr1 =
          allProgress.find(
            (p) => p.tournamentId === t.id && p.userId === p1Id,
          ) ?? null;
        const pr2 =
          allProgress.find(
            (p) => p.tournamentId === t.id && p.userId === p2Id,
          ) ?? null;
    ...
    const progressEntityByTid = new Map<number, TournamentProgress>();
    ...
    if (allIds.length > 0) {
      ...
      for (const p of effectiveProgressList) {
        ...
        if (p.userId === userId) {
          progressByTid.set(p.tournamentId, data);
          progressEntityByTid.set(p.tournamentId, p);
        }
        if (!progressByTidAndUser.has(p.tournamentId)) {
          progressByTidAndUser.set(p.tournamentId, new Map());
        }
    ...
    const toItem = (
      ...
      const visibleStageTotals = getVisibleStageTotalsForProgress(
        progressEntityByTid.get(t.id),
        t,
        questionsByTournamentId,
      );

      if (round === 'semi') {
        questionsTotal = visibleStageTotals.semi.totalQuestions;
        questionsAnsweredInRound = visibleStageTotals.semi.answeredQuestions;
        correctAnswersInRound = visibleStageTotals.semi.correct;
      } else {
        questionsTotal = visibleStageTotals.final.totalQuestions;
        questionsAnsweredInRound = visibleStageTotals.final.answeredQuestions;
        correctAnswersInRound = visibleStageTotals.final.correct;
      }
  ...
  async getTournamentState(
    ...
    if (
      ...
      if (tournament.status === TournamentStatus.FINISHED) {
        ...
        const normalizedProgressState = normalizeProgressSnapshot(
          progressState,
          true,
        );
        const wonSemi =
    ...
    const normalizedProgress = normalizeProgressSnapshot(progress, true);
    const opponentSlot = getOpponentSlot(playerSlot, order.length);
    ...
    if (opponent && progress) {
      ...
      const normalizedOppProgress = normalizeProgressSnapshot(
        oppProgress,
        false,
      );
  ...
  private async loadTournamentQuestions(
    tournamentId: number,
  ): Promise<Question[]> {
  ...
  private async prepareTrainingStateMutations(
    ...
    const tournament = await this.getTrainingTournamentForUser(
      userId,
      tournamentId,
    );
    const progress = await this.tournamentProgressRepository.findOne({
    ...
    let questions =
      await this.ensureSemifinalQuestionRoundsPrepared(tournament);
  ...
  private normalizeAnswersChosen(val: unknown): number[] {
    return normalizeChosenAnswers(val);
  }
  ...
  private async ensureTournamentPlayersLoaded(
    ...
    const missingIds = (tournament.playerOrder ?? []).filter(
      (id): id is number =>
        Number.isInteger(id) && id > 0 && !loadedIds.has(id),
    );
  ...
  async getTournamentBracket(
    ...
    const toPlayer = (p: User, isLoser?: boolean) => {
      ...
      const inFinalPhase = prog
        ...
        : false;
      const semiAnswered = visibleStageTotals.semi.answeredQuestions;
      const semiScore =
        semiAnswered > 0 ? visibleStageTotals.semi.correct : undefined;
    ...
    const enrichFinalPlayer = (
      ...
      const finalAnswered = visibleStageTotals.final.answeredQuestions;
      const finalCorrect = visibleStageTotals.final.correct;
