...
export function TournamentBracketModal(props: {
  ...
  setBracketPlayerTooltip: React.Dispatch<React.SetStateAction<BracketPlayerTooltipData | null>>;
  ...
  return (
    <div className="bracket-overlay" onClick={() => !bracketLoading && onClose()}>
      <div
      ...
      >
        <div className="bracket-modal-header">
          ...
          <button type="button" className="bracket-close" onClick={onClose} aria-label="Закрыть">×</button>
        </div>
        ...
        {bracketView && (
          <div className={`bracket-grid ${variant === 'admin' ? 'bracket-grid--admin ' : ''}${bracketBlocksEqualized ? 'bracket-blocks-equalized' : ''}`}>
...
export function TournamentQuestionsModal(props: {
  ...
  return (
    <div className="questions-review-overlay" onClick={closeQuestionsReview}>
      <div className={`questions-review-modal questions-review-modal--${variant}`} onClick={(e) => e.stopPropagation()}>
        ...
        {questionsReviewData && (() => {
          ...
          const myAnswerTitle = variant === 'player' ? 'Мой ответ' : 'Ответ игрока';
          const noAnswersText = variant === 'player' ? 'Вы не ответили ни на один вопрос в этом раунде.' : 'Игрок не ответил ни на один вопрос в этом раунде.';

          return (
            <>
              <div className="qr-legend">
                <span className="qr-legend-item"><span className="qr-check qr-check--correct">✓</span> Правильный ответ</span>
                <span className="qr-legend-item"><span className="qr-check qr-check--mine">✓</span> {myAnswerTitle}</span>
                          ...
                          <table className="qr-table">
                            <thead>
                              <tr>
                                ...
                                <th className="qr-th-icon qr-th-mine" title={myAnswerTitle}>{noMyAnswer ? <span className="qr-cross">✗</span> : '✓'}</th>
